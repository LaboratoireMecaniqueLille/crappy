...
class Path(ABC):
  ...
  def __init__(self, *_, **__) -> None:
    ...
    :class:`~crappy.blocks.generator_path.meta_path.path.Path`), they can be
    parsed here using the :meth:`parse_condition` method. See the code of
    ...
    previous :class:`~crappy.blocks.generator_path.meta_path.path.Path` was
    sent, and the ``self.last_cmd`` stores the value of the last command of the
    previous :class:`~crappy.blocks.generator_path.meta_path.path.Path`.
  ...
  def get_cmd(self, data: dict[str, list]) -> float | None:
    ...
    :meth:`~crappy.blocks.meta_block.block.Block.recv_all_data` for more
    information on the format of this dict.
    ...
    * any other criteria
    ...
    :class:`~crappy.blocks.generator_path.meta_path.path.Path` is done and
    should hand over to the next one, it must raise a :exc:`StopIteration`
    exception. Again, the choice to raise that exception can be motivated by
    the current time, a condition as generated by :meth:`parse_condition`, or
    any other criteria.
  ...
  def parse_condition(self,
    ...
    last command. See
    :meth:`~crappy.blocks.meta_block.block.Block.recv_all_data` for information
    on the structure of this data. Based on the input data, it returns
    :obj:`True` if the condition is met, and :obj:`False` otherwise.
    ...
    the next :class:`~crappy.blocks.generator_path.meta_path.path.Path`.
