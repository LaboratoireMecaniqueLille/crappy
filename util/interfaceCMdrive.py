...
class Interface(Frame):
  ...
  def __init__(self, root, **kwargs):
    ...
    self.location = Button(sous_fra1, text="Examine location",
        command=self.examineLocation, width=10)
    ...
    self.moveZeroButton = Button(sous_fra3_2, text="Move Home",
        command=self.moveZero, width=15)  # create move home button
    # disable the state of the button
    ...
    if self.motionType.get() == "" or self.position.get() == "":
      ...
      if self.actuatorName == "CmDrive":
        if self.motionType.get() == '1':
          self.actuator.set_position(int(self.position.get()), None, 'absolute')
          print 'MA mode'
        else:
          self.actuator.set_position(int(self.position.get()), None, 'relative')
          print 'MR mode'
      else:
        self.actuator.set_position(int(self.position.get()), int(self.speed.get()))
