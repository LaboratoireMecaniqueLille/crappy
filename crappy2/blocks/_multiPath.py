...
##  @defgroup MultiPath MultiPath
# @{

## @file _multiPath.py
# @brief Children class of MasterBlock. Use it for traction-torsion testing.
#
...
class MultiPath(MasterBlock):
    ...
    def __init__(self, path=None, send_freq=400, dmin=22, dmax=25, default_G=71 * 10 ** 9, default_E=196 * 10 ** 9,
                 repeat=False, normal_speed=6.6 * 10 ** (-4)):
        """
        This block is specific for use in traction-torsion testing.

        You need to define a path to follow, with the available waveform. Unlike the
    ...
    def send(self, traction, torsion):
        Array = OrderedDict(
            zip(['t(s)', 'def(%)', 'dist(deg)', 'def_plast(%)', 'E(Pa)', 'G(Pa)', 'status', 'relative_eps_tot'],
                [time.time() - self.t0, traction, torsion, self.plastic_def, self.E, self.G, self.status,
                 self._relative_total_def]))
        # print "array : ",Array
        try:
    ...
    def detection(self):
        ...
        while self.detection_step < 16:  # while we doesn't have 16 points for the plasticity surface
            ...
            if self.first_step_of_detection:
                ...
                try:
                    first_vector = np.subtract(self.initial_position, self.position) / np.linalg.norm(
                        np.subtract(self.initial_position, self.position))
                ...
                if tilt:
                    angles += np.pi / 16.
                self.target_positions = [np.cos(angles) * 10, np.sin(angles) * 10 * np.sqrt(
                    3)]  # normalized positions, multiplied by 10 to be unreachable
            ...
            if self.detection_substep == 0:  # if going toward plasticity
                try:
                    self.goto(
                        [self.target_positions[0][self.detection_step], self.target_positions[1][self.detection_step]],
                        mode="towards")  # move a little to detetc the plasticity surface
                    # print self.target_positions
                    self.get_position()
                    self._relative_total_def = np.sqrt((self.position[0] - self.central_position[0]) ** 2 + (
                    (self.position[1] - self.central_position[1]) ** 2) / 3.)
                    if self.status > self.detection_step:
                        self.FIFO.insert(0, np.sqrt(
                            (self.position[0] - self.eps0 - (self.effort[0] - self.F0) / self.E) ** 2 + (
                            (self.position[1] - self.gam0 - (self.effort[1] - self.C0) / self.G) ** 2) / 3.))
                        if len(self.FIFO) > 10:
                            self.FIFO.pop()
                ...
                if current_step["waveform"] == "detection":
                    ...
                    if current_step["mode"] == "total_def":
                        ...
                        if current_step["position"][0] == 0:  # torsion pure
                            ...
                            value = np.sqrt((self.position[0] - eps0 - (self.effort[0] - F0) / self.default_E) ** 2 + (
                            (self.position[1] - gam0 - (self.effort[1] - C0) / self.default_G) ** 2) / 3.)
                        FIFO.insert(0, value)
                        while np.mean(FIFO) < current_step["target"]:
                            ...
                            if current_step["position"][0] == 0:  # torsion pure
                                ...
                                value = np.sqrt(
                                    (self.position[0] - eps0 - (self.effort[0] - F0) / self.default_E) ** 2 + (
                                    (self.position[1] - gam0 - (self.effort[1] - C0) / self.default_G) ** 2) / 3.)
                            FIFO.insert(0, value)
                            if len(FIFO) > 10:
                                FIFO.pop()
                    ...
                    if np.linalg.norm(
                            np.subtract([self.f(0)], self.position)) > self.offset:  # if not in starting position
