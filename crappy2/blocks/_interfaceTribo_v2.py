...
class ThreadedTask(threading.Thread):
    """
    Class used to make a new thread when calling it.
    """
...
class InterfaceTribo(Frame, MasterBlock):
    """A brand new window to control tribometer"""

    def __init__(self, root, variateur_tribo, labjack, labjack_hydrau, conditioner):
        MasterBlock.__init__(self)

        def file_path_update(var):
            """Method to update the file_path inserted by user"""
            self.outputs[1].send(var)
            self.outputs[1].send(var)

        def actuator_speed_update(var):
            """Method to update the speed value in window"""
        ...
        def actuator_position_update(var):
            """Method to update the position value in the window"""
            if var < self.PositionMin:
                ...
                self.variateur_tribo.actuator.go_position(-int(var))
                print 'Position demandée = ' + str(var)

        def actuator_force_update(var):
        ...
        try:
            ...
            self.filepath = "/home/tribo/save_dir/log.txt"
            self.mode = 'Mode Position'
            self.variateur_tribo = variateur_tribo
            ...
            self.ChangeModeButton = Button(frameManualAuto, text="SIMULATION D'INERTIE",
                                           command=self.change_manual_auto)
            ...
            self.ActuatorSpeedSpinbox.bind('<Return>', lambda event: actuator_speed_update(self.ActuatorSpeedVar.get()))
            ...
            self.ActuatorForceSpinbox.bind('<Return>', lambda event: actuator_force_update(self.ActuatorForceVar.get()))
            ...
            self.ActuatorPositionSpinbox.bind('<Return>',
                                              lambda event: actuator_position_update(self.ActuatorPositionVar.get()))
    ...
    def on_closing(self):
        if tkMessageBox.askokcancel("Quit", "Do you want to quit?"):
            self.variateur_tribo.actuator.set_mode_position()
            self.variateur_tribo.actuator.stop_motor()
            self.labjack.set_cmd(0)
            self.labjack.set_cmd_ram(-41, 46000)
            self.labjack.set_cmd_ram(0, 46002)
            self.labjack.close()
            self.variateur_tribo.close()
            self.root.destroy()
