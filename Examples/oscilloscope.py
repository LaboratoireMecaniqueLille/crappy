...
try:
########################################### Creating objects
	instronSensor=crappy.sensor.ComediSensor(device='/dev/comedi0',channels=[0,1,2,3,4,5],gain=[2,100000,0.01*2.,1000,1,1])
...
########################################### Creating blocks
	...
	stream=crappy.blocks.MeasureComediByStep(instronSensor, labels=['t(s)','def(%)','F(N)','dist(deg)','C(Nm)','dep(mm)','ang(deg)'], freq=200.)
	...
	graph=crappy.blocks.Grapher("static",('t(s)','t(s)'))
