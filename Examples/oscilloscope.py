...
try:
    ...
    # sensor = crappy.sensor.LabJackSensor(channels=[0], gain=[1], chan_range=10, mode="streamer", scanRate=10,
    #                                       scansPerRead=5)
    # sensor = crappy.sensor.LabJackSensor(channels=[0], gain=1, resolution=12, chan_range=10, mode="single")  #
    ...
    stream = crappy.blocks.MeasureByStep(sensor, labels=['t(s)', 'AIN0', 'AIN1', 'AIN2', 'AIN3'], freq=100)
    ...
    graph = crappy.blocks.Grapher(('t(s)', 'AIN0'), ('t(s)', 'AIN1'), ('t(s)', 'AIN2'), ('t(s)', 'AIN3'), length=50)
    ...
    link1 = crappy.links.Link(name="stream_to_signalGenerator")
