...
path = {
    "type":"ramp",
    "speed":SPEED/60, # Convert to mm/s (in ramps, speed is always in unit/s)
...
# make_zero asks the board to read the value at the beginning and substract
...
# Because TDACx is ncessarily an output and AINx an input, the block will guess
# the direction of each channel. If ambiguous it must be specified with
...
# to use. The block will instanciate it itself ! No need to create it here
