...
#class condition_def(crappy.links.MetaCondition): # cette condition ser ta remettre a 0 la def du videoextenso un e fois passé un certain seuil d'effort
...
class condition_defposition(crappy.links.MetaCondition):
	def __init__(self):
	...
	def evaluate(self,value):
		#print "top1"
		recv=self.external_trigger.recv(blocking=False)  # lecture non bloquante de l'effort : si on a pas dépassé l'offset en effort, aucune info n'a été envoyé
		#print "top2"
		if recv is not None: # si on a reçu quelquechose:
			self.val0=value['position'][0] # on récupère la valeur de position au moment du assage de l'offset en effort
			self.start=True
			#print "top3"
		if self.start:
			...
			#print "top4"
			return value
		else: # si ona rien reçu encore, envoie la value avec def=0 pour avoir quand meme une def a envoyer au signalgenerator.
			value['Eyy(%)'] = pd.Series((0), index=value.index)# def =0 tant qu'on a pas passé l'offset en effort
			###value['Exx(%)']=pd.Series((100*((value['position'][0])/self.l0-1)), index=value.index) ## décommente ça si tu veux quand meme calculer la def au début, en prenant l0 = distance initiale (7 mm)
			#print "top5"
			return value
...
########################################### Creating blocks
	...
	save_effort=crappy.blocks.Saver("/home/biotens/Bureau/Annie/essais_rupture_3-11/E2124_rat10_LB_rupture_effort_1.txt")
	graph_effort=crappy.blocks.Grapher("dynamic",('t(s)','F(N)'))
	
	compacter_extenso=crappy.blocks.Compacter(90)
	save_extenso=crappy.blocks.Saver("/home/biotens/Bureau/Annie/essais_rupture_3-11/E2124_rat10_LB_rupture_extenso_1.txt")
	...
	signalGenerator=crappy.blocks.SignalGenerator(path=[{"waveform":"limit","gain":1,"cycles":3,"phase":0,"lower_limit":[0.09,'F(N)'],"upper_limit":[5,'Eyy(%)']},
							...
							{"waveform":"limit","gain":1,"cycles":0.5,"phase":0,"lower_limit":[0.09,'F(N)'],"upper_limit":[90,'F(N)']}],
							send_freq=5,repeat=False,labels=['t(s)','signal','cycle'])
	
	
	biotens=crappy.blocks.CommandBiotens(biotens_technicals=[biotensTech],speed=5)
	compacter_position=crappy.blocks.Compacter(5)
	save_position=crappy.blocks.Saver("/home/biotens/Bureau/Annie/essais_rupture_3-11/E2124_rat10_LB_rupture_position_1.txt")
...
########################################### Creating links
	...
	link10=crappy.links.Link(condition_defposition())
	link11=crappy.links.Link()
	link112=crappy.links.Link()
	link100=crappy.links.Link(condition_defposition())
	
	link12=crappy.links.Link(condition_offset())  
	link10.add_external_trigger(link12)
	link13=crappy.links.Link(condition_offset())
...
########################################### Linking objects
	...
	biotens.add_output(link10) #c'est ici qu'on récupère la position du moteur non ? --> ici on recupere la position pour la sauvegarder
	biotens.add_output(link100) # on dédouble la sortie : ici on la récupere pour l'asservissement
	
	# on passe les 2 links 10 et 100 par la condition "condition_defposition" pour calculer la def et la remttre a 0 quand l'effort depasse l'offset.
