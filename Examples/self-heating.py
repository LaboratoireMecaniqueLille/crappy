...
# WARNING : This should also be modified to keep the tension value aside the evaluated temperature, so you can modify the calibration law in post-treatement. (in condition_calib class)
...
########################################### Creating links
	...
	link1=crappy.links.Link(condition=condition_calib())#crappy.links.Filter(labels=['T','T2','T3'], mode='mean', size=100),condition_calib(),condition_sub()
	link2=crappy.links.Link(condition=eval_stress())
