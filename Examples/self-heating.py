# The file is to be used for self-heating measurement. As the control off the machine is not set,
# you can use it on any test (traction, torsion, TTC).
# For the Labjack, it is recommended to either wait 3-4H before strating the test,
# or use some hardware to remove the cold-junction-issue.
# it is also recommended on the Labjck to use a resolution adapted to yur measure :
# 			- up to 12 if you need precision, 8/9 if you need speed.
...
class ConditionCalib(crappy.links.MetaCondition):
    def __init__(self):
        ...
        self.coeff_T2 = [-5.37671145e+18, -1.08133635e+16, 6.54277247e+12, 4.91194020e+09, -1.47818544e+06,
                         1.91198815e+04, 2.77679671e+01]
        self.coeff_T3 = [-7.70172105e+18, -8.61309675e+15, 6.53446611e+12, 4.30990401e+09, -1.34687552e+06,
                         1.90248560e+04, 2.80630626e+01]
        self.T_init_FIFO = []
    ...
    def evaluate(self, value):
        ...
        value.update(
            {'T_mean': calc, 'T_cal': T - self.T_init, 'T2_cal': T2 - self.T2_init, 'T3_cal': T3 - self.T3_init})
        return value
...
try:
    ...
    graph = crappy.blocks.Grapher(('t(s)', 'T_cal'), ('t(s)', 'T2_cal'), ('t(s)', 'T3_cal'),
                                   ('t(s)', 'T_mean'))  # ,('t(s)','T_mean')
    graph_effort = crappy.blocks.Grapher(('t(s)', 'C(Nm)'))
    graph_effort2 = crappy.blocks.Grapher(('t(s)', 'tau(Pa)'))
    ...
    # condition=[crappy.links.Filter(labels=['V'],mode="median",size=50),crappy2.links.Filter(labels=['t(s)'],mode="mean",size=50)]
