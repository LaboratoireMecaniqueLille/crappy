...
generator = crappy.blocks.Generator(path=[
  {'type':'cyclic_ramp','speed1':speed,'condition1':'setpoint>1',
    'speed2':-speed,'condition2':'setpoint<0','cycles':100},
    ],cmd_label='setpoint')
...
dis = crappy.blocks.DISCorrel('',input_label='frame',show_image=True,
    labels=['t(s)','x','y','measured_Exx(%)','measured_Eyy(%)'],verbose=True)
...
# Here the setpoint is coming from the generator
# and the feedback is the strain measured with the DIC
...
pid = crappy.blocks.PID(kp=0.5,ki=2,kd=0.05,
    target_label='setpoint', input_label='measured_Exx(%)', send_terms=True)
...
# channels of the PID (P, I and D). This can be useful when adjusting the gains
