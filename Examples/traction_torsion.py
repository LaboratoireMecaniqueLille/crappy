...
########################################### Creating objects
	...
	instronSensor=crappy.sensor.ComediSensor(device='/dev/comedi0',channels=[0,1,2,3],gain=[50,10,45,1000]) # 10 times the gain on the machine if you go through an usb dux sigma
...
########################################### Creating blocks
	...
	stream=crappy.blocks.MeasureComediByStep(t0,instronSensor,labels=['t(s)','dep(mm)','f(kN)','angle(deg)','C(Nm)'])
	...
	torsion=crappy.blocks.PathGenerator(t0, send_freq=400, actuator=cmd_torsion, waveform=['sinus','triangle','sinus'], freq=[0.5,2,1], time_cycles=[10,10,10], amplitude=[1,2,4], offset=[0,0,0], phase=[np.pi,np.pi,np.pi], repeat=True)
	compacter=crappy.blocks.Compacter(100)
	save=crappy.blocks.Saver("/home/essais-2015-1/Bureau/t_dep_F_angle_C.txt")
	graph=crappy.blocks.Grapher("dynamic",(0,1),(0,3))
	graph_stat=crappy.blocks.Grapher("static",(0,1),(0,3))
