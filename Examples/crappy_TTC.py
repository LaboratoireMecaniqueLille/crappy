...
if __name__ == '__main__':
    try:
        ...
        # cmd_traction=crappy.actuator.ComediActuator(device='/dev/comedi1', subdevice=1, channel=1, range_num=0,
        #  gain=8*100, offset=0)
        ...
        graph_traction = crappy.blocks.Grapher(('sigma(Pa)', 'tau(Pa)'),length=0)
        graph_torsion = crappy.blocks.Grapher(('def(%)', 'dist(deg)'),length=0)
        ...
        graph_path = crappy.blocks.Grapher(('t(s)', 'def(%)'))
        graph_path2 = crappy.blocks.Grapher(('t(s)', 'dist(deg)'))
