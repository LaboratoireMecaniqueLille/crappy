...
class ConditionSignal(crappy2.links.MetaCondition):
...
if __name__ == '__main__':
    try:
        ...
        instronSensor = crappy2.sensor.ComediSensor(device='/dev/comedi0', channels=[0, 1, 2, 3],
                                                    ...
                                                    offset=offset)
        # cmd_traction=crappy2.actuator.ComediActuator(device='/dev/comedi1', subdevice=1, channel=1,
        ...
        # with Labjack:
        cmd_traction = crappy2.actuator.LabJackActuator(channel="TDAC2", gain=10 / 100000., offset=0)
        cmd_torsion = crappy2.actuator.LabJackActuator(channel="TDAC3", gain=10 / 1000., offset=0)
        ...
        # multipath=crappy2.blocks.MultiPath(path=[{"waveform":"circle","gain":0.0006,"cycles":200,"offset":[-0.0006,0]},
        ...
        multipath_traction = crappy2.blocks.SignalGenerator(path=[
        ...
        multipath_torsion = crappy2.blocks.SignalGenerator(path=[
            {'waveform': 'sinus', 'time': 1000000, 'phase': np.pi / 2., 'amplitude': ((125 * 10 ** 6) / rmoy) * I,
             'offset': 0, 'freq': 1}],
                                                          send_freq=400, repeat=False,
                                                          labels=['t(s)', 'torsion', 'cycle'])
        compacter = crappy2.blocks.Compacter(1)
        ttc = crappy2.blocks.CommandComedi([cmd_traction, cmd_torsion], signal_label=['traction', 'torsion'])
        ...
        # graph_path=crappy2.blocks.Grapher("dynamic",('t(s)','traction'))
