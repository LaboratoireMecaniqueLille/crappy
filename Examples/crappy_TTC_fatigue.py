...
if __name__ == '__main__':
    try:
        ...
        instronSensor = crappy.sensor.ComediSensor(device='/dev/comedi0', channels=[0, 1, 2, 3],
        ...
        instronSensor = crappy.sensor.ComediSensor(device='/dev/comedi0', channels=[0, 1, 2, 3],
                                                   ...
                                                   offset=offset)
        # cmd_traction=crappy.actuator.ComediActuator(device='/dev/comedi1', subdevice=1, channel=1,
        #                                             range_num=0, gain=8*100, offset=0)
        # cmd_torsion=crappy.actuator.ComediActuator(device='/dev/comedi1', subdevice=1, channel=2, range_num=0,
        #                                            gain=8*100/2., offset=0) # divide dist by 2 for the testing machine

        # with Labjack:
        cmd_traction = crappy.actuator.LabJackActuator(channel="TDAC2", gain=10 / 100000., offset=0)
        cmd_torsion = crappy.actuator.LabJackActuator(channel="TDAC3", gain=10 / 1000., offset=0)
        ...
        # stream=crappy.blocks.StreamerComedi(instronSensor, labels=['t(s)','def(%)','F(N)','dist(deg)','C(Nm)'],
        #                                     freq=200.)
        ...
        multipath_traction = crappy.blocks.SignalGenerator(path=[
            {'waveform': 'sinus', 'time': 1000000, 'phase': 0, 'amplitude': (125 * 10 ** 6) * surface,
             'offset': (-125 * 10 ** 6) * surface, 'freq': 1}],
                                                           send_freq=400, repeat=False,
                                                           labels=['t(s)', 'traction', 'cycle'])
        multipath_torsion = crappy.blocks.SignalGenerator(path=[
            {'waveform': 'sinus', 'time': 1000000, 'phase': np.pi / 2., 'amplitude': ((125 * 10 ** 6) / rmoy) * I,
             'offset': 0, 'freq': 1}],
                                                          send_freq=400, repeat=False,
                                                          labels=['t(s)', 'torsion', 'cycle'])
        compacter = crappy.blocks.Compacter(1)
        ttc = crappy.blocks.CommandComedi([cmd_traction, cmd_torsion], signal_label=['traction', 'torsion'])
        # torsion=crappy.blocks.CommandComedi([cmd_torsion])

        compacter_data = crappy.blocks.Compacter(100)
        save = crappy.blocks.Saver("/home/corentin/Bureau/data_labjack_contrainte_instron1000_cycles.txt")
        graph_traction = crappy.blocks.Grapher("dynamic", ('sigma(Pa)', 'tau(Pa)'))  # ('sigma(Pa)','tau(Pa)')
        graph_torsion = crappy.blocks.Grapher("dynamic", ('def(%)', 'dist(deg)'))  # ('def(%)','dist(deg)')
        ...
        # graph_path=crappy.blocks.Grapher("dynamic",('t(s)','traction'))
        ...
        link1 = crappy.links.Link(EvalStrain(k=1))
        link2 = crappy.links.Link()
        link3 = crappy.links.Link()
        link4 = crappy.links.Link(EvalStrain(k=2))
        ...
        multipath_traction.add_output(link5)
        multipath_torsion.add_output(link9)
        compacter.add_input(link5)
