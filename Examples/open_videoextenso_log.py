# This file provide a way to open the files saved by crappy videoxtenso.
# Please don't modify this file directly but make a copy for your own usage.
...
if __name__ == '__main__':
    ...
    def get_data(file_path, columns):
        # load the file in str format ### put here the column you need
        # converters = ({0: lambda s: float(s.lstrip("['")) or float(s.lstrip("[['"))})
        a = np.loadtxt(file_path, dtype=str, usecols=columns, skiprows=1, delimiter="' '",
                       converters={0: lambda s: float(s.split("'")[-1]),
                                   1: lambda s: (s.strip().replace("  ", ",").replace(" ", "")),
                                   2: lambda s: (s.replace("  ", ",").replace(" ", "")),
                                   3: lambda s: float(s),
                                   4: lambda s: float(s.split("'")[0])})
