...
def get_field(s,h,w):
  if s == 'x':
    ...
    return (np.concatenate((np.linspace(-1, 1, w,
            dtype=np.float32)[np.newaxis, :],)*h, axis=0),
            zeros(h,w))
  elif s == 'eyy':
    return (zeros(h,w),
            np.concatenate((np.linspace(-1, 1, h,
            dtype=np.float32)[:, np.newaxis],)*w, axis=1))
  elif s == 'exy':
    return (np.concatenate((np.linspace(-1, 1, h,
            dtype=np.float32)[:, np.newaxis],)*w, axis=1),
            zeros(h,w))
  elif s == 'eyx':
    return (zeros(h,w),
            np.concatenate((np.linspace(-1, 1, w,
            dtype=np.float32)[np.newaxis, :],)*h, axis=0))
  elif s == 'exy2':
    return ((w/(w*h)**.5)*np.concatenate((np.linspace(-1, 1, h,
            dtype=np.float32)[:, np.newaxis],)*w, axis=1),
            (h/(w*h)**.5)*np.concatenate((np.linspace(-1, 1, w,
            dtype=np.float32)[np.newaxis, :],)*h, axis=0))
