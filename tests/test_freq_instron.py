...
########################################### Creating objects
	...
	#sensor=crappy2.sensor.ComediSensor(device='/dev/comedi0',channels=[0,1,2,3],gain=[0.02,100000,0.01*2.,500]) # dist is multiplied by 2 to be correct
	sensor_effort=crappy2.sensor.ComediSensor(device='/dev/comedi0', channels=[0, 1], gain=[10, 500])
	#sensor=crappy2.sensor.ComediSensor(device='/dev/comedi0',channels=[0,1,2],gain=[1,1,1],offset=[0,0,0]) #262*10**-6,180*10**-6,169*10**-6
	...
	#sensor=crappy2.sensor.LabJackSensor(channels=[0,1,2],gain=[1,1,1],resolution=9,chan_range=0.01,mode="single",scanRate=100,scansPerRead=1) # dist is multiplied by 2 to be correct
...
########################################### Creating blocks
	...
	#save=crappy2.blocks.Saver("/home/corentin/Bureau/thermocouples/torsion_2.txt")
	#graph=crappy2.blocks.Grapher("dynamic",('t(s)','T'),('t(s)','T2'),('t(s)','T3')) #,('t(s)','T_mean')
	graph_effort=crappy2.blocks.Grapher("dynamic", ('t(s)', 'C(Nm)'))
	graph_effort2=crappy2.blocks.Grapher("dynamic", ('t(s)', 'angle'))
	save_effort=crappy2.blocks.Saver("/home/corentin/Bureau/test_mesure_instron2.txt")
...
########################################### Creating links
	...
	link1=crappy2.links.Link()#crappy2.links.Filter(labels=['T','T2','T3'], mode='mean', size=100),ConditionCalib(),condition_sub()
...
########################################### Linking objects
	...
	#compacter.add_output(anglelink3)
