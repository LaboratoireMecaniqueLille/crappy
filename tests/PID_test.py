...
try:
########################################### Creating objects
	instronSensor=crappy2.sensor.ComediSensor(device='/dev/comedi0', channels=[0], gain=[10])
...
########################################### Creating blocks
	...
	stream=crappy2.blocks.MeasureByStep(instronSensor, labels=['t(s)', 'signal_mesure'], freq=400)
	#stream=crappy2.blocks.MeasureByStep(sensor,labels=['t(s)','signal'])
	...
	traction=crappy2.blocks.SignalGenerator(path=[{"waveform": "sinus", "time":100, "phase":0, "amplitude":1, "offset":0, "freq":1}],
	...
	send_output=crappy2.blocks.CommandComedi([cmd_traction])
	compacter=crappy2.blocks.Compacter(200)
	#compacter2=crappy2.blocks.Compacter(400)
	#save=crappy2.blocks.Saver("/home/ilyesse/Bureau/delete_me3.txt")
	graph=crappy2.blocks.Grapher("dynamic", ('t(s)', 'signal_mesure'))
...
########################################### Creating links
	...
	link2=crappy2.links.Link(condition=crappy2.links.PID(1.01, 0.00, 0, label_consigne='signal', label_retour='signal_mesure', outMin=-8, outMax=8))
