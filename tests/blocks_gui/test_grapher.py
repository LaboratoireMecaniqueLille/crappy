...
class TestGrapher(BlockTestBase):
  ...
  def setUp(self) -> None:
    """Tracks Graphers, suppresses notices, and closes stale figures."""

    super().setUp()
    self._warning_patcher = patch.object(grapher_module, 'warn')
    self._warning_patcher.start()
    self._graphers: list[Grapher] = list()
    grapher_module.plt.close('all')

  def tearDown(self) -> None:
    """Closes plotting windows before resetting the Block class state."""
  ...
  def _prepare_pyqt_grapher(self,
                            *labels: tuple[str, str],
                            **kwargs) -> tuple[Grapher, TestBlock]:
    """Creates a linked Grapher and prepares its PyQtGraph window."""

    kwargs.setdefault('plotter', 'pyqt')
  ...
  def test_labels_are_validated(self) -> None:
    ...
    self.assertEqual(Grapher(('x', 'y'))._graph_labels,
                     (('x', 'y'),))

  def test_constructor_validates_plotter_and_backend(self) -> None:
    """Checks plotter selection and plotter-specific backends."""

    self.assertEqual(Grapher(('x', 'y'))._plotter, 'pyqt')
    self.assertEqual(Grapher(('x', 'y'), backend='PyQt6')._backend,
                     'PyQt6')
    self.assertEqual(Grapher(('x', 'y'), plotter='mpl',
                             backend='TkAgg')._backend, 'TkAgg')

    with self.assertRaises(ValueError):
      Grapher(('x', 'y'), plotter='invalid')
    with self.assertRaises(ValueError):
      Grapher(('x', 'y'), backend='TkAgg')
    with self.assertRaises(ValueError):
      Grapher(('x', 'y'), plotter='mpl', backend='PyQt6')
  ...
  def test_constructor_validates_limits_and_refresh_frequency(self) -> None:
    ...
    with self.assertRaises(ValueError):
      Grapher(('x', 'y'), length=3)

    with self.assertRaises(ValueError):
      Grapher(('x', 'y'), upd_freq=11, freq=10)

    self.assertEqual(Grapher(('x', 'y'), length=True,
                             max_pt=None)._length, 1)
    self.assertEqual(Grapher(('x', 'y'), max_pt=True)._max_pt, 1)
    self.assertEqual(Grapher(('x', 'y'), upd_freq=True)._upd_freq, 1.0)

    for value in (float('nan'), float('inf')):
      with self.subTest(value=value):
        with self.assertRaises(ValueError):
          Grapher(('x', 'y'), upd_freq=value, freq=None)
  ...
  def test_prepare_builds_pyqtgraph_window_and_curves(self) -> None:
    """Checks the PyQtGraph application, plot, and curve objects."""

    grapher, _ = self._prepare_pyqt_grapher(
        ('x', 'y'), ('time', 'z'), window_size=(3.5, 2.25))

    self.assertIsNotNone(grapher._qt_app)
    self.assertIsNotNone(grapher._qt_plot)
    self.assertEqual(len(grapher._qt_curves), 2)
    self.assertEqual({curve.name() for curve in grapher._qt_curves},
                     {'y', 'z'})
    self.assertEqual(grapher._factor, [1, 1])
    self.assertEqual(grapher._counter, [0, 0])
  ...
  def test_pyqtgraph_loop_updates_curve_data(self) -> None:
    ...
    grapher, _ = self._prepare_pyqt_grapher(('x', 'y'), upd_freq=None)
    grapher.recv_all_data_raw = lambda: [{'x': [1, 2], 'y': [3, 4]}]

    grapher.loop()

    x_data, y_data = grapher._qt_curves[0].getData()
    self.assertEqual(x_data.tolist(), [1, 2])
    self.assertEqual(y_data.tolist(), [3, 4])

  def test_pyqtgraph_uses_requested_binding(self) -> None:
    """Checks that the selected Qt binding reaches PyQtGraph."""

    with patch.dict(grapher_module.os.environ):
      self._prepare_pyqt_grapher(('x', 'y'), backend='PyQt6')

      self.assertEqual(grapher_module.os.environ['PYQTGRAPH_QT_LIB'], 'PyQt6')

  def test_pyqtgraph_processes_events_once_after_curve_updates(self) -> None:
    """Checks that multi-curve updates are rendered as one batch."""

    grapher, _ = self._prepare_pyqt_grapher(
        ('x', 'y'), ('x', 'z'), upd_freq=None)
    grapher.recv_all_data_raw = lambda: [
      {'x': [1, 2], 'y': [3, 4], 'z': [5, 6]},
    ]

    with patch.object(grapher, '_qt_app',
                      wraps=grapher._qt_app) as qt_app:
      grapher.loop()

    self.assertEqual(qt_app.processEvents.call_count, 2)
  ...
  def test_clear_resets_pyqtgraph_curves_and_resampling_state(self) -> None:
    """Checks the PyQtGraph c-key callback behavior."""
    ...
    grapher._on_press_pyqt()

    x_data, y_data = grapher._qt_curves[0].getData()
    self.assertTrue(x_data is None or not len(x_data))
    self.assertTrue(y_data is None or not len(y_data))
    self.assertEqual(grapher._factor, [1])
  ...
  def test_finish_before_prepare_does_not_close_another_figure(self) -> None:
    ...
    grapher = Grapher(('x', 'y'), plotter='mpl')
