...
The DownSampler Modifier returns only the last received data point every n data
points it receives from its upstream Block. This Modifier is useful for
reducing the data rate in a given Link.
...
sine wave is 1 Hz and the frequency of the Generator Block is twice as much the
number of points of downsampling, the value as returned by the DownSampler
Modifier is always close to 1 or -1, the peaks of the sine wave.
...
Modifier. This demo ends after 22s. You can also hit CTRL+C to stop it earlier,
...
if __name__ == '__main__':
  ...
  # of the Generator Block is twice as much the number of points of
  # downsampling, only values close to 1 and -1 are received.
