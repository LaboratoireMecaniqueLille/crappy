...
"""
This example demonstrates the use of the Mean Modifier. It does not require any
specific hardware to run, but necessitates the matplotlib Python module to be
installed.

The Mean Modifier averages all the data it receives from its upstream Block.
Once it has averaged enough data points, it sends the calculated average values
to the downstream Block and enters the next averaging loop. It therefore
reduces the data rate on a given Link. This Modifier is useful for reducing the
data rate in a given Link while still preserving as much information as
possible.
...
displays it averaged by a Mean Modifier. Because the frequency of the sine wave
and that of the averaging are the same, the average value as returned by the
Mean Modifier is always close to 0.
...
if __name__ == '__main__':
  ...
  # averaging, only values close to 0 are received. A point is received once
  # every second, because the Mean Modifier only outputs data once every 30
  # received points
  ...
  crappy.link(gen, graph_avg,
              # Adding a Mean Modifier for calculating the average of the sine
