...
"""
Demonstrates the use of the Canvas block.
...
class TestBlock(crappy.Block):
  """A simple block for demonstrating the use of the Canvas block.

  Simply updates the values on the Canvas at each loop.
  """
...
if __name__ == "__main__":
  ...
  # Canvas. Here we add 'dot_text' elements.
  ...
  # Canvas. We simply give it a list containing the label of each element.
  update_block = TestBlock([d['label'] for d in options])

  # Now adding a 'time' element to the Canvas. The 'text' and 'label' keys
  # are not mandatory and do not affect the Canvas.
  options.append({'type': 'time',
                  'coord': (80, 1000),
                  'text': '',
                  'label': ''})

  # The block containing the Canvas
  canvas = crappy.blocks.Canvas(image_path=img_path, draw=options,
                                color_range=(20, 300), title="Temperatures")

  # Linking and starting the blocks
  crappy.link(update_block, canvas)
  crappy.start()
