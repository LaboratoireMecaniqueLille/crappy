...
especially useful for plotting one label from one Block against another label
from a different Block. It is also useful for recording data from several
Blocks in a single file with the Recorder Block, as the Recorder can only save
...
frequencies and sent to a Multiplexer for interpolation. The interpolated data
is then sent to a Grapher for display.

After starting this script, just watch how the two interpolated labels are
displayed at the same frequency by the Grapher, although they originally had
very different frequencies. Try changing the frequencies to see the result.
This demo ends after 22 seconds. Click the stop button to end the demo early.
"""
...
if __name__ == '__main__':
  ...
  # way, both the 'cmd_slow' and 'cmd_fast' labels that originally had very
  ...
  # identical, although they were originally very different
