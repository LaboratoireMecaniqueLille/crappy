...
Two Generators produce labels at different frequencies, while another Generator
produces a random reference label. They are all sent to a Synchronizer for
interpolation, and the interpolated data is sent to a Grapher for display.

After starting this script, just watch how the two interpolated labels are now
synchronized on the time base of the reference label. The Grapher then displays
them all at the same frequency, although they originally had very different
frequencies. Try changing the frequencies to see the result. This demo ends
after 22 seconds. Click the stop button to end the demo early.
"""
...
class RandomPath(crappy.blocks.generator_path.meta_path.Path):
  ...
  def __init__(self,
    ...
    """Set the arguments and initialize the parent class.
  ...
  def get_cmd(self, data: dict[str, list]) -> float | None:
    """Return a randomly generated value after sleeping a random number of
    seconds."""
...
if __name__ == '__main__':
  ...
  gen_ref = crappy.blocks.Generator(
      ...
      ({'type': 'RandomPath', 'time_range': (0.150, 0.500),
        'value_range': (-1, 1)},),
  ...
  gen_sig_1 = crappy.blocks.Generator(
      # Generating a sine signal of period 20 seconds and amplitude 2
