...
different value once the end condition is met. However, unlike in the other
Generator examples, the stop condition is not one of the standard ones defined
in Crappy but is instead an arbitrary callable defined by the user. Here, the
condition checks whether a given file exists, but it could use any other kind
of condition.

After starting this script, create a file named 'test.txt' in the current
working directory. See how the signal value changes once the file is created.
Once you delete the newly created file, the test should then end due to the
second custom condition. You can also end this demo earlier by clicking on the
...
def file_exists(data):
  """Returns True if 'test.txt' exists in the current working directory and
  False otherwise.
  ...
  Args:
    data: The data received by the Generator Block since its last loop. The
      keys are the labels, and each value is a list containing all the received
...
def file_does_not_exist(data):
  """Returns False if 'test.txt' exists in the current working directory and
  True otherwise.
...
if __name__ == '__main__':
  ...
  gen = crappy.blocks.Generator(
      ...
      # associated with the 'condition' key
