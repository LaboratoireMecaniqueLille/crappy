...
class InterfaceTomo4D(MasterBlock, Frame):
    ...
    def __init__(self, root, Actuator, TransX, TransY, Load, **kwargs):
        ...
        frameActuator = Frame(frameActuatorInfo, width=400, borderwidth=2,
                              relief=GROOVE)  # create a frame in canvas(p1)
        ...
        frameActuatorPosition = Frame(frameInfo, width=200, borderwidth=2,
                                      relief=GROOVE)  # create a frame in canvas(p1)
        ...
        self.cameraSelection = Tix.StringVar()  # create a variable, it will contain ser selection
        ...
        self.ActuatorPositionActualisationGhostButton = Button(self.root, text="+1", bg="yellow",
                                                               command=self.ActuatorPositionUpdate())
        # LoadDeclaration
        ...
        self.ActuatorSpeedPIDSpinbox = Spinbox(framePID, from_=0, to=1, increment=0.001,
                                               textvariable=self.ActuatorSpeedVarPID)
        ...
        self.ActuatorPositionTareButton = Button(frameActuatorPosition, text="TarePosition",
                                                 command=self.Actuator.TarePosition)
        # ActuatorSpeedButton
    ...
    def RealtimePloter(self):
        try:
            ...
            if len(self.tbuffered) > 600:  # on n'affiche pas plus de 10 minutes ~ 6000 points
                ...
                self.ActuatorPositionValuebuffered = self.ActuatorPositionValuebuffered[1:len(self.tbuffered)] + [
                    self.ActuatorPositionValue]
    ...
    def SendOutput(self):
        try:
            ...
            Array = OrderedDict(
                zip(['t(s)', 'position (mm)', 'F(N)', 'RecordFlag', 'RecordPath', 'PIDflag', 'SpeedPID', 'LoadPID'],
                    [self.t, self.ActuatorPositionValue, self.LoadValue, self.flag, self.filepath, self.PIDflag,
                     self.SpeedPID, float(self.LoadVarPID.get())]))
            ...
            pass
        self.recursive_func_log.append(
            self.root.after(100, self.SendOutput))  # Defini l'intervalle d'envoie des données vers l'extérieur
