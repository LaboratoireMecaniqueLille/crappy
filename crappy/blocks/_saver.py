...
class Saver(MasterBlock):
    ...
    def __init__(self, log_file, *args, **kwargs):
        ...
        self.output_format = '.' + log_file.rsplit('.', 1)[-1]  # To define if it's a CSV, or a simple TXT where lists are saved directly.
        ...
        if self.stamp:
            if self.stamp == 'date':
                ...
                self.nb_identical_files = [value.count(self.log_name) for _, value in
                                           enumerate(self.nb_files_folder)]
