...
class Interface(Frame,MasterBlock):
	...
	def __init__(self, root,VariateurTribo, comediOut):#,link1,link2):
		try:	
			...
			self.ActuatorSpeedSpinbox= Spinbox(self.frameSpeed, from_=self.SpeedMin, to=self.SpeedMax, increment=1, textvariable= self.ActuatorSpeedVar )
			...
			self.ActuatorForceSpinbox=Spinbox(self.frameForcePos, from_=self.ForceMin, to=self.ForceMax, increment=1, textvariable= self.ActuatorForceVar )
			...
			self.ActuatorPositionSpinbox=Spinbox(self.frameForcePos, from_=self.PositionMin, to=self.PositionMax, increment=100, textvariable= self.ActuatorPositionVar )
			self.ActuatorPositionSpinbox.bind('<Return>', lambda event: ActuatorPositionUpdate(self.ActuatorPositionVar.get()))




			#init Declaration
			self.InitButton = Button(self.frameInitialisation, text='Initialisation Variateur',command=self.initialisation)
			...
			self.StartExperimentButton = Button(self.frameProperties, text="Démarrer l'essai", command=self.startExperiment)
			self.AbortExperimentButton = Button(self.frameProperties, text="Arrêter l'essai", command=self.stopExperiment)
