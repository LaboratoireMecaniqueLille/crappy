...
def barycenter_opencv(recv_):
	...
	while True:
		...
		# 	print "pid: ", os.getpid()
		# 	image1=sitk.GetImageFromArray(image)
		# 	sitk.WriteImage(image1,"img_videoExtensoFAIL%i.tiff"%os.getpid())
		# else:
		# 	image1=sitk.GetImageFromArray(image)
		# 	sitk.WriteImage(image1,"img_videoExtenso%i.tiff"%os.getpid())
		# 	while(M['m00']==0.0):
	...
	def main(self):
		...
		while True:
			try:	
				...
				# image1=sitk.GetImageFromArray(image)
				# sitk.WriteImage(image1,"img_videoExtenso%.5d.tiff" %j)
				...
				# image1=sitk.GetImageFromArray(image)
				# sitk.WriteImage(image1,"img_videoExtensoORGINAL.tiff")
				for i in range(0,self.NumOfReg): # for each spot, calulate the news coordinates of the center, based on previous coordinate and border.
					...
					# image1=sitk.GetImageFromArray(image[int(self.minx[i])-1:int(self.maxx[i])+1,int(self.miny[i])-1:int(self.maxy[i])+1])
					# sitk.WriteImage(image1,"img_videoExtensoBEFORE%i.tiff"%i)
