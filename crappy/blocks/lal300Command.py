...
class CommandLal300(MasterBlock):
    """
    Programme permettant de realiser des essais cycliques avec parametres de deplacement,
    de cycles et de vitesse variables
    """
    ...
    def main(self):
        try:
            ...
            while etape < len(self.param['CYCLES']):  ## Pour le mode sans limite en effort
                block = 0
                print "Etape= :", etape
                while block < self.param['CYCLES'][
                    etape]:  # Block= cycles locaux, reinitialise a chaque franchissement d'etapes
                    try:
                        ########################### Commande moteur en mode position: Traction ###########################

                        # position=self.technical.sensor.check_disp()
                        inputs_var = self.inputs[
                            0].recv()  # recuperation des variables en entree du block CommandLal300
                        position = (-10000 * inputs_var[
                            'dep(mm)'])  # definition d'une variable position (en unite moteur) via le deplacement LVDT en mm
                        ...
                        self.param['ACC'], self.param['SPEED'][etape], self.param['FORCE'], self.param[
                            'ENTREE_VERIN']))  # Ecriture de la consigne en mode vitesse avec les parametres adequats (cf documentation moteur SMAC Lal 300)
                        while position >= (self.param['ETIRE'][
                                               etape]):  # Comparaison entre variable position et param ETIRE de l'etape en cours lorsque condition vraie: si position=ETIRE sortie de boucle
                        ...
                        Array = OrderedDict(zip(['t(s)', 'cycle', 'position'],
                                                [s, cycle, position]))  # Stockage des variables dans une matrice
                    ...
                    SerialException) as s:  # Gestion des exceptions SerialException et ValueError lors d'une mauvaise lecture du port serie: le programme alerte et poursuit
