...
class DaqmxSensor(InOut):
  ...
  def get_data(self, nb_points=1):
    ...
    try:
      ...
      data = numpy.zeros((nb_points + 1,), dtype=numpy.float64)
      t0 = time.time()
      DAQmxReadAnalogF64(self.taskHandle, nb_points + 1, 10.0,
                         DAQmx_Val_GroupByChannel, data,
                         nb_points + 1, byref(self.read), None)  # DAQmx Read Code
      t = time.time()
      # DAQmx Stop Code
