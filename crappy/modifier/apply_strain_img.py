...
class Apply_strain_img(Modifier):
  ...
  def __init__(self,img,exx_label='Exx(%)',eyy_label='Eyy(%)',
    ...
    self.xx,self.yy = np.meshgrid(range(w),range(h))

  def evaluate(self,d):
    exx,eyy = d[self.lexx]/100,d[self.leyy]/100
    d[self.img_label] = cv2.remap(self.img,
      (self.xx-(exx/(1+exx))*self.exx).astype(np.float32),
      (self.yy-(eyy/(1+eyy))*self.eyy).astype(np.float32),1)
    return d
